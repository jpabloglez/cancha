...
function defaultFilters(): Filters {
  return {
    stat: "ppg", league: "", seasonFrom: "", seasonTo: "",
    position: "", minGames: 20, limit: 10,
  };
}

function filtersFromProps(initial?: Partial<Filters>): Filters {
...
function exportCsv(rows: AllTimeLeader[], stat: string) {
  ...
  const lines = rows.map((r, i) =>
    [
      i + 1, r.playerName, r.primaryPosition ?? "", r.nationality ?? "",
      r.totalGames, r.seasonsCount, r.leagues.join("|"),
      r.ppg.toFixed(1), r.rpg.toFixed(1), r.apg.toFixed(1),
      r.spg.toFixed(1), r.bpg.toFixed(1), r.topg.toFixed(1),
      pct(r.twoPercent), pct(r.threePercent), pct(r.ftPercent),
      r.per != null ? r.per.toFixed(1) : "",
      pct(r.tsPercent), r.totalPoints.toFixed(0),
      r.totalRebounds.toFixed(0), r.totalAssists.toFixed(0),
    ].join(",")
  );
    ...
    <tr className="border-t border-zinc-100 dark:border-zinc-800">
      {Array.from({ length: cols }).map((_, i) => (
        <td key={i} className="px-3 py-3">
          <div className="h-3 animate-pulse rounded bg-zinc-200 dark:bg-zinc-700" style={{ width: i === 1 ? "120px" : "40px" }} />
        </td>
      ))}
    </tr>
  );
...
export function AllTimeLeaderboard({
  ...
  const [filters, setFilters] = useState<Filters>(() => filtersFromProps(initialFilters));
  ...
  // Push current filter state to URL so the view is shareable/bookmarkable.
  const syncUrl = useCallback(
    (f: Filters, off: number) => {
      const p = new URLSearchParams();
      if (f.stat !== "ppg") p.set("stat", f.stat);
      if (f.league) p.set("league", f.league);
      if (f.seasonFrom) p.set("from", f.seasonFrom);
      if (f.seasonTo) p.set("to", f.seasonTo);
      if (f.position) p.set("pos", f.position);
      if (f.minGames !== 20) p.set("min", String(f.minGames));
      if (f.limit !== 10) p.set("n", String(f.limit));
        ...
        <table className="w-full text-sm">
          <thead className="border-b border-zinc-200 bg-zinc-50 text-left text-xs font-semibold uppercase tracking-wider text-zinc-500 dark:border-zinc-700 dark:bg-zinc-800/60 dark:text-zinc-400">
            <tr>
              ...
              <th className="px-3 py-2 text-right" title="% tiro de 2">T2%</th>
              <th className="px-3 py-2 text-right" title="% tiro de 3">T3%</th>
              <th className="px-3 py-2 text-right" title="% tiros libres">TL%</th>
          ...
          <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
            {loading && rows.length === 0
              ? Array.from({ length: filters.limit > 10 ? 10 : filters.limit }).map((_, i) => (
              ...
              : rows.length === 0 && !loading
                ...
                : rows.map((r, i) => {
                    ...
                    return (
                      <tr
                        key={r.playerId}
                        className={`transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800/40 ${loading ? "opacity-40" : ""}`}
                      >
                        ...
                        <td className="px-3 py-2"><PositionBadge position={r.primaryPosition} short /></td>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between text-sm text-zinc-500">
          ...
          <div className="flex gap-2">
            ...
            <button
              disabled={offset + filters.limit >= count || loading}
